  ...
  return (
    <section className={className}>
      <div className="mb-6 text-center">
        <h2 className="text-2xl font-semibold text-brand-400 sm:text-3xl">{title}</h2>
        <p className="mx-auto mt-2 max-w-3xl text-sm text-text-muted sm:text-base">{subtitle}</p>
      </div>
        ...
        {HIGHLIGHTS.map((item) => {
          ...
          return (
            <Card
              ...
              padding="lg"
              hoverable
              className="h-full rounded-2xl border-brand-100 bg-white shadow-sm">
              <div className="flex h-full flex-col gap-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-50 text-brand-400">
                  <Icon size={22} strokeWidth={2.2} />
                </div>
                <div className="space-y-2">
                  <h3 className="text-lg font-semibold text-text-default">{item.title}</h3>
