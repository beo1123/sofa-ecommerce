  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <Grid cols={1} responsive={{ md: 2 }} gap="lg">
          <GridItem className="flex flex-col justify-center gap-md">
            <h1 className="text-4xl font-bold">
              Nội thất sang trọng cho ngôi nhà hiện đại
            </h1>
            <p className="text-text-muted">
              Khám phá những thiết kế nội thất tinh tế được chế tác từ vật liệu
              cao cấp và tay nghề thủ công.
            </p>
      ...
      <section id="bestsellers" className="p-xl  bg-bg-page">
        ...
        <Grid cols={1} responsive={{ sm: 2, md: 3, lg: 4 }} gap="lg">
          {Array.from({ length: 8 }).map((_, i) => (
            <GridItem
              key={i}
              className="bg-bg-muted rounded-md h-64 flex items-center justify-center"
            >
              Sản phẩm {i + 1}
            </GridItem>
          ))}
      ...
      <section id="browse-by-rooms" className="p-xl rounded-lg bg-brand-100">
        ...
        <Grid cols={2} responsive={{ sm: 3, lg: 6 }} gap="md">
          {[
            ...
            <GridItem
              key={room}
              className="bg-white shadow rounded-md p-lg text-center hover:shadow-lg transition"
            >
              {room}
            </GridItem>
          ))}
      ...
      <section id="top-selling" className="p-xl rounded-lg mt-5 bg-bg-muted">
        ...
        <Grid cols={1} responsive={{ sm: 2, md: 3, lg: 4 }} gap="lg">
          {Array.from({ length: 8 }).map((_, i) => (
            <GridItem
              key={i}
              className="bg-white rounded-md h-64 flex items-center justify-center shadow"
            >
      ...
      <section id="promos" className="p-xl rounded-lg mt-5 bg-brand-200">
        <h2 className="text-2xl font-semibold mb-lg text-white">
          Ưu đãi đặc biệt
        </h2>
        <Grid cols={1} responsive={{ sm: 2, lg: 3 }} gap="md">
          {[
            "Giảm 30% Sofa",
            "Khuyến mãi đèn chiếu sáng tuần này",
            "Sale nội thất ngoài trời",
          ].map((offer) => (
            <GridItem
              key={offer}
        ...
        <Grid cols={1} responsive={{ md: 2 }} gap="lg">
          ...
          <GridItem className="flex flex-col justify-center gap-md">
            <h3 className="text-2xl font-semibold">
              Nội thất sang trọng bắt đầu từ chất liệu cao cấp nhất
            </h3>
            <p className="text-text-muted">
              Triết lý của chúng tôi là kết hợp thiết kế tinh tế và vật liệu bền
              vững để tạo ra sản phẩm nội thất trường tồn qua nhiều thế hệ.
            </p>
