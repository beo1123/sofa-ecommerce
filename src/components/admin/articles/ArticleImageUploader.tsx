...
interface ArticleImageUploaderProps {
  ...
  folder?: string;
  /** Present when editing an existing article – enables immediate DB deletion */
  articleId?: number;
}
...
export default function ArticleImageUploader({
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError(e?.response?.data?.error?.message ?? "Upload thất bại");
    } finally {
      setUploading(false);
      e.target.value = "";
    }
  ...
  const handleRemove = async (index: number) => {
    ...
    try {
      // If the image has a DB id and we know the article, delete from DB + Cloudinary
      if (image.id && articleId) {
        await axiosClient.delete(`/admin/articles/${articleId}/images/${image.id}`);
      } else {
        // Newly uploaded image – delete only from Cloudinary
        await axiosClient.delete("/admin/upload", { data: { url: image.url } });
      }
    } catch (err: unknown) {
      const e = err as { response?: { data?: { error?: { message?: string } } } };
      setError(e?.response?.data?.error?.message ?? "Xóa ảnh thất bại");
      // Still remove from local state even if delete fails (Cloudinary may be unreachable)
    } finally {
            ...
            <div
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                ...
                <button
                  ...
                  className="p-1 bg-red-500 text-white rounded shadow hover:bg-red-600 disabled:opacity-60">
                  {deletingIndex === i ? <Loader2 size={14} className="animate-spin" /> : <X size={14} />}
                </button>
              </div>
